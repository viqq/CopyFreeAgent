...
public class SocialNetworkAuthenticationManager implements AuthenticationManager {
    ...
    public Authentication authenticate(Authentication auth) throws EmailDidNotRegistredException, BadCredentialsException {
        String email = auth.getName();
        SocialProfile profile = (SocialProfile) auth.getCredentials();
        User user = userDao.findByEmail(email);
        if (user == null) {
            throw new EmailDidNotRegistredException("Email " + email + "has not registred yet");
        }
